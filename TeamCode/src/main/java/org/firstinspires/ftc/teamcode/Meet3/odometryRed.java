...
@TeleOp(name = "100% Competition TeleOp - RED", group = "COMPETITION")
public class odometryRed extends OpMode {
    ...
    private void handleHybridAimingAndShooting() {
        ...
        if (tag != null && tag.ftcPose != null) {
            ...
            double odometryAngleRad = distanceLocalization.getTargetTurretAngle(robotPose, true);
            turretAngleDeg = Math.toDegrees(odometryAngleRad) - tag.ftcPose.yaw;
        } else {
            aimingMode = "ODOMETRY FALLBACK";
            distanceToGoal = distanceLocalization.getDistanceToGoal(robotPose, true);
            double odometryAngleRad = distanceLocalization.getTargetTurretAngle(robotPose, true);
        ...
        if (robotX >= -75 && robotX <= -24 && robotY >= -25 && robotY <= -1) {
            targetRPM = 1570;
            aimingMode = "ODOMETRY OVERRIDE - Far";
        } else if (robotX >= -75 && robotX <= -54 && robotY >= -91 && robotY <= -61) {
            targetRPM = 1380;
    ...
    private AprilTagDetection getRedTag() {
        ...
        if (detections != null) {
            for (AprilTagDetection detection : detections) {
                if (detection.id == RED_GOAL_TAG_ID) return detection;
            }
        }
        return null;
    }
