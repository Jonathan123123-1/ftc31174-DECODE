...
@TeleOp(name = "TEST: All Systems Blue", group = "TEST")
public class AllSystemsGamepad1Test extends LinearOpMode {
    ...
    /* ================= SHOOTER CONSTANTS ================= */
    private static final double FAR_VELOCITY = 1530;   // 1570 RPM
    private static final double CLOSE_VELOCITY = 1210; // 1250 RPM
    ...
    private double shooterTargetVelocity = FAR_VELOCITY;
    ...
    private static final double LimeLight_kP = 0.009;//0.012
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            /* ---------- SHOOTER ZONE SELECT ---------- */
            if (gamepad1.x) shooterTargetVelocity = FAR_VELOCITY;
            if (gamepad1.y) shooterTargetVelocity = CLOSE_VELOCITY;
            ...
            if(gamepad1.left_bumper && result.isValid()&&Math.abs(result.getTx())>3){
                turretMotor.setPower(-result.getTx()*LimeLight_kP+Math.signum(-result.getTx())*.08);
            }else{
